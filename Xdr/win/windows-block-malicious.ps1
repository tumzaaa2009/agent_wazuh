...
# [2026-08-24 fix] double JSON decode ปลอดภัยขึ้น — decode รอบสองเฉพาะเมื่อได้ string ซ้อนอยู่
# (Wazuh ส่ง single-wrapped ปกติ, integrations daemon อาจส่ง double-wrapped)
$INPUT_ARRAY = $INPUT_JSON | ConvertFrom-Json
if ($INPUT_ARRAY -is [string] -or $INPUT_ARRAY -isnot [PSCustomObject]) {
    try { $INPUT_ARRAY = ($INPUT_ARRAY | Out-String).Trim() | ConvertFrom-Json } catch { }
}
...
$ipWhitelist = @()   # เพิ่ม IP ภายในที่ต้องการยกเว้น เช่น "10.0.192.15"

function IsPrivateIP([string]$ip) {
    if ($ip -match '^(10\.|172\.(1[6-9]|2\d|3[01])\.|192\.168\.|127\.|169\.254\.|::1|fe80)') { return $true }
    return $false
}

function IsDomainWhitelisted([string]$domain) {
    $wl = @('wazuh.com','windowsupdate.com','microsoft.com')   # domain ที่ห้าม block
    foreach ($w in $wl) { if ($domain -like "*$w") { return $true } }
    return $false
}
...
if ($mispType -and $mispValue) {
    ...
    # ─── fallback: Suricata EVE path (จาก network sensor / soc_edge rules) ──
    $eveDst = $INPUT_ARRAY."parameters"."alert"."data"."dstip"
    if (-not $eveDst) { $eveDst = $INPUT_ARRAY."parameters"."alert"."data"."dest_ip" }
    if ($eveDst) {
        $IOCeventid = '3'
        $IOCtype    = 'ip-dst'
        $IOCvalue   = $eveDst
