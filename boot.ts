...
// chown/chmod by NAME fails inside the container ("invalid user: 'wazuh:wazuh'") because
// name resolution happens in the container's own nsswitch, not the host's.
// Numeric uid:gid works regardless, since filesystem permissions are numeric underneath.
// Override via env if the host's wazuh uid/gid ever differ (check with: getent passwd wazuh).
const WAZUH_UID = process.env.WAZUH_UID || "115";
const WAZUH_GID = process.env.WAZUH_GID || "125";
...
// Always logs the result (success or failure).
